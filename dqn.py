...
def learn(env,
    ...
    Q_y = rew_t_ph + (1-done_mask_ph)*gamma*tf.reduce_max(target_q, axis = 1)
    Q_s = tf.reduce_sum(q * tf.one_hot(act_t_ph, num_actions), axis = 1)
    total_error = tf.reduce_mean(tf.square(Q_s - Q_y))
    q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func')
    ...
    for t in itertools.count():
        ...
        if (np.random.random() < exploration.value(t)) or not model_initialized:
            ...
            action_q_values = session.run(q, feed_dict = {obs_t_ph: [q_input]})[0]
            action = np.argmax(action_q_values)
        
        # Perform the action in the env
        new_s, new_r, done, info = env.step(action)
        
        # save the result in replay_buffer
        replay_buffer.store_effect(idx, action, new_r, done)
        last_obs = new_s
